...
def numeric_feature_visualization(data, features):
    ...
    charts_numeric = [
        alt.Chart(data)
        .transform_density(
            feature, as_=[feature, "density"], groupby=["survival_status"]
        )
        ...
        .encode(
            ...
            color=alt.Color(
                "survival_status:O", legend=alt.Legend(title="Survival Status")
            ).scale(scheme="dark2"),
        )
        .properties(
            width=180, height=180, title=f"Density Plot of {feature} by Survival Status"
        )
...
def fee_paid_visualization(data, feature="FeePaid"):
    ...
    """
    chart = (
        alt.Chart(data[data["FeePaid"] <= 1000])
        .transform_density(
            feature, as_=[feature, "density"], groupby=["survival_status"]
        )
        .mark_area(opacity=0.5)
        ...
        .properties(
            width=200, height=200, title=f"Density Plot of {feature} by Survival Status"
        )
    )
...
def num_of_employee_visualization(data, feature="NumberofEmployees"):
    ...
    """
    chart = (
        alt.Chart(data[data["NumberofEmployees"] <= 20])
        .transform_density(
            "NumberofEmployees",
...
def categorical_feature_visualization(data, feature):
    ...
    chart = (
        ...
        .facet(column="survival_status:O")
        .resolve_scale(x="independent", y="independent")
        .properties(title=f"Frequency of {feature} by Survival Status")
    )
...
def varianced_categorical_feature_visualization(data, feature="BusinessType"):
    ...
    chart = (
        ...
        .facet(column="survival_status:O", columns=2)
        .properties(title=f"Frequency of Top 20 {feature} by Survival Status")
    )
...
@click.option(
    "--merged_data_path"
)  # merged_data_path = 'data/processed/business_econ.csv'
