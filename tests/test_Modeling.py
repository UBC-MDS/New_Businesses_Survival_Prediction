...
def test_split_x_y_return_dataframe():
    assert isinstance(X_df, pd.DataFrame), "`split_x_y` return X is not a pandas data frame. Should return a pandas data frame"
    assert isinstance(y_df, pd.Series), "`split_x_y` return y is not a pandas data frame. Should return a pandas data frame"
