...
def test_econ_datacleaning_return_dataframe():
    assert isinstance(econ_df, pd.DataFrame), "`fetch_business_license` should return a pandas dataframe"
...
def test_merge_return_dataframe():
    assert isinstance(business_econ_df, pd.DataFrame), "`merge_business_econ_by_year` should return a pandas dataframe"